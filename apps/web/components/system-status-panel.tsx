...
export function SystemStatusPanel({
  ...
  const workerStatusLabel =
    workerOnline == null
      ? t("systemStatusWorkerNotConfigured")
      : workerOnline
        ? t("systemStatusWorkerOnline")
        : t("systemStatusWorkerOffline");
  ...
  return (
    <section className="mb-10 rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
      <h2 className="text-sm font-semibold">{t("systemStatusTitle")}</h2>
      <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-3">
        <div>
          <dt className="text-[var(--muted)]">{t("systemStatusWorker")}</dt>
          <dd className="mt-1 flex flex-wrap items-center gap-2">
            ...
            {workerOnline && workerLatencyMs != null ? (
              <span className="text-xs text-[var(--muted)]">
                {t("systemStatusWorkerResponse", {
                  duration: formatDurationMs(workerLatencyMs),
                })}
              </span>
            ) : null}
          </dd>
        ...
        <div>
          <dt className="text-[var(--muted)]">{t("systemStatusFailedPolls")}</dt>
          <dd className="mt-1 font-medium">{failedPollCount24h}</dd>
        </div>
        <div>
          <dt className="text-[var(--muted)]">{t("systemStatusAvgPoll")}</dt>
          <dd className="mt-1 font-medium">{formatAveragePollDuration(averagePollDurationMs)}</dd>
        </div>
      </dl>
      {workerOnline === false ? (
        <p className="mt-3 text-xs text-[var(--muted)]">{t("systemStatusFreeTierHint")}</p>
      ) : null}
    </section>
  );
}
